...
{
  var state1 = gameStates[gameStates.length - 2];
  var state2 = gameStates[gameStates.length - 1];
  giveMethods([state1,state2]);
  var player = state2.players[controlId];
  if (!player.alive)
  {
    return;
  }
  var minDist = 100000;
  var playerClosest = -1;
  for (var i in state2.players)
  {
    if (i == controlId || !state2.players[i].alive)
    {
      continue;
    }
    var dist = state2.players[i].pos.distanceTo(player.pos);
    if (dist < minDist)
    {
      minDist = dist;
      playerClosest = i;
  ...
  var outAng;
  if (playerClosest == -1 || !controlsBundle.keys[32])
  {
    outAng = controlsBundle.ang;
  }
  else if (player.weapon == -1)
  {
    outAng = state2.players[playerClosest].pos.angTo(state2.players[controlId].pos);
  }
  else{
    var framesToHit = 1 + minDist/(framesPerTick * state2.weapons[player.weapon].bulletSpeed);
    var predictPos = state2.players[controlId].pos.add(state2.players[controlId].pos.subtract(state1.players[controlId].pos).multiply(framesToHit));
    var predictTargetPos = state2.players[playerClosest].pos.add(state2.players[playerClosest].pos.subtract(state1.players[playerClosest].pos).multiply(framesToHit));
    
    outAng = predictTargetPos.angTo(predictPos);
    console.log(framesToHit);
  }
  socket.emit('mousemove', outAng);
}
